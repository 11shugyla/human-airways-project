...
for snap_path in snap_files:
    ...
    for region in key_regions:
        ...
        row[region + "_mean_Pa"] = round(p.mean(), 2)
        row[region + "_max_Pa"]  = round(p.max(), 2)
        row[region + "_min_Pa"]  = round(p.min(), 2)
    row["global_mean_Pa"] = round(pressure.mean(), 2)
    row["global_max_Pa"]  = round(pressure.max(), 2)
    row["global_min_Pa"]  = round(pressure.min(), 2)
    results.append(row)
